...
SEMA_SRCS := $(sort $(wildcard tests/sema/*_src.r2))

.PHONY: all clean test test_lexer test_parser test_sema
...
test: test_lexer test_parser test_sema
...
test_sema: $(TARGET)
	@if [ -z "$(SEMA_SRCS)" ]; then echo "No sema tests found"; exit 1; fi; \
	passed=0; \
	failed=0; \
	for src in $(SEMA_SRCS); do \
		base="$${src%_src.r2}"; \
		expected_out="$${base}_result.txt"; \
		expected_err="$${base}_stderr.txt"; \
		expected_status="$${base}_status.txt"; \
		tmp_out="$$(mktemp)"; \
		tmp_err="$$(mktemp)"; \
		"$(TARGET)" --dump-symbols "$$src" >"$$tmp_out" 2>"$$tmp_err"; \
		actual_status=$$?; \
		want_status=0; \
		if [ -f "$$expected_status" ]; then want_status="$$(cat "$$expected_status")"; fi; \
		ok=1; \
